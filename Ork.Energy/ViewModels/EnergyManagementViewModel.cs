...
namespace Ork.Energy.ViewModels
{
  ...
  {
    private readonly BindableCollection<ConsumerGroupViewModel> m_ConsumerGroups = new BindableCollection<ConsumerGroupViewModel>();
    ...
    private IEnumerable<ConsumerViewModel> FilteredConsumers
    {
      ...
      get
      {
        ...
        return filteredConsumers.Where(c => c.Model.Distributor.Equals(SelectedDistributor.Model) && c.Model.ConsumerGroup.Equals(SelectedConsumerGroup.Model));
      }
    }
    ...
    public void Save(object dataContext)
    {
      ...
      {
        Dialogs.ShowMessageBox("Dieser Verbraucher existiert bereits in der Datenbank.", "Fehler", MessageBoxOptions.Ok, box => OpenEditor(dataContext));
      }
      else
      {
        CloseEditor();
    ...
    {
      return m_Repository.Consumers.Any(c => c.Name == consumer.Name && c.Distributor == consumer.Distributor && c != consumer);
    }
